...
function backtest(bars,p) {
  ...
  const rs=rsi(closes,p.period);
  const rs6=rsi(closes,6),rs12=rsi(closes,12),rs24=rsi(closes,24);
  ...
  for(let i=p.period+1;i<bars.length;i++){
    ...
    pending=null;
    if(!shares&&rs[i-1]>=p.buy&&rs[i]<p.buy)pending={type:"buy",signalDate:bars[i].date};
    if(shares&&rs[i-1]<=p.sell&&rs[i]>p.sell)pending={type:"sell",signalDate:bars[i].date};
  ...
  const indicators=bars.map((bar,i)=>({date:bar.date,rsi6:rs6[i],rsi12:rs12[i],rsi24:rs24[i]}))
    .filter((x)=>x.rsi6!==null||x.rsi12!==null||x.rsi24!==null);
  return{curve,benchmark,markers,indicators,tradeRecords,stats:stats(curve,closedTrades)};
}
...
export async function runBankBacktest(form) {
  ...
  const portfolio=stats(combined,trades);
  ...
  return{meta:{stockCount:ok.length,stockCode:ok[0].code,stockName:ok[0].name,dataSource:ok[0].source,start,end,generatedAt:new Date().toLocaleString("zh-CN")},
    portfolio,equity,markers:ok[0].test.markers,indicators:ok[0].test.indicators,
