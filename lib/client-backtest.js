...
async function universe(target) {
  ...
  try {
    const json = await jsonp("https://push2.eastmoney.com/api/qt/clist/get", {
      pn:1,pz:100,po:1,np:1,fltt:2,invt:2,fid:"f3",fs:"b:BK0475",fields:"f12,f14"
    });
...
export async function runBankBacktest(form) {
  ...
  const stocks=await universe(form.target);
  const loaded=await mapLimited(stocks,6,async([code,name])=>{try{const bars=await history(code,start,end);if(bars.length<=p.period+5)throw new Error("有效周线不足");return{code,name,test:backtest(bars,p)};}catch(e){return{code,name,error:e.message};}});
  const ok=loaded.filter((x)=>!x.error);if(!ok.length)throw new Error("行情源暂时不可用，请检查网络后重试。");
  ...
  return{meta:{stockCount:ok.length,start,end,generatedAt:new Date().toLocaleString("zh-CN")},
